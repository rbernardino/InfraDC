...
try
{
  ...
  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/rbernardino/InfraDC/master/InfraDC/ConfigurationData.psd1' -UseBasicParsing -OutFile $configDataFilePath
  ...
  describe 'New-TestEnvironment' {
    ...
    $resourceGroup = 'dsclabdcs'
    $azrvm = 'dc01'

    ## Create a shared session for all of the calls we need to make.
    BeforeAll {
      $vm = Get-AzureRmVm -Name $expectedDomainControllerName -ResourceGroupName $resourceGroup
      $ipAddress = (Get-AzureRmPublicIpAddress -ResourceGroupName $resourceGroup -Name "$azrvm-public-ip").IpAddress
      ...
      $script:sharedSession = New-PSSession @pssessionParams
    }
